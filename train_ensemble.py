...
def MSE(y, x, logvar2):
    ...
    custom_loss = torch.mean(((y - x) ** 2) / (torch.exp(logvar2)) + logvar2)
...
class FNet:
    ...
    def train(self, path, batch_size, num_epochs, checkpoints_dir, ensamble):
        ...
        for epoch in range(num_epochs):
            ...
            plt.subplot(151), plt.imshow(np.squeeze((out_data.data[slice_idx, :, :, :]).cpu()), cmap='gray', vmin=0,
                                         vmax=255)
            ...
            plt.subplot(153), plt.imshow(np.squeeze((recon_batch.data[slice_idx, :, :, :]).cpu()), cmap='gray', vmin=0,
                                         vmax=255)
    ...
    def _create_architecture(self):
        ...
        self.optimizer = optim.RMSprop(self.model.parameters(), lr=LEARNING_RATE, eps=1e-08, alpha=0.9,
                                       weight_decay=0)  # does not have rho=RMS_WEIGHT_DECAY I used alpha=RMS_WEIGHT_DECAY
